# HU-16 - Generar Archivo Periódico para Socios
...
Este archivo se genera íntegramente en inglés para el socio internacional y tiene tres secciones. La primera resume los accesos por departamento y área, con las columnas: Department, Area, Total, Authorized, Denied, Unregistered, Suspended y % Authorized. La segunda muestra la distribución por día, con las columnas: Day, Total, Authorized, Denied, Unregistered y Suspended. La tercera es el log detallado de accesos (solo ingresos) con las columnas: Date/Time, Employee, Position, Employee ID, Department, Area y Result; este log expone únicamente el nombre, el cargo y el código de empleado, sin tipos ni números de documento ni ningún otro dato personal.

En Excel el archivo se entrega en tres pestañas (By Area, By Day y Access Log); en CSV las secciones van apiladas con encabezados; en PDF como tablas secuenciales. El nombre del archivo descargado es partner_periodic_file_{anio}_{mes}.{ext}.
...
| 1.4 | 2026-08-23 | | | Archivo en inglés; tercera sección Access Log (nombre, cargo y código de empleado, sin documentos); Excel en 3 pestañas; nombre partner_periodic_file | |

## Estado de Implementación

- **Backend**: ✓ — `POST /api/reportes/archivo-periodico` con tres secciones en inglés: agregación **por departamento × área** (Department, Area, Total, Authorized, Denied, Unregistered, Suspended, % Authorized), **distribución por día** (Day, Total, Authorized, Denied, Unregistered, Suspended) y **Access Log** detallado (Date/Time, Employee, Position, Employee ID, Department, Area, Result — solo nombre, cargo y código de empleado; sin tipo/número de documento). Filtro opcional `departmentNames`; **solo ingresos** (las salidas EXIT quedan fuera). Formatos CSV (secciones apiladas), EXCEL (3 pestañas: By Area, By Day, Access Log) y PDF (3 tablas). `POST /api/reportes/archivo-periodico/preview` devuelve `areaRows`/`dayRows` en JSON para la vista previa. Error si el período no tiene registros. Descarga como `partner_periodic_file_{anio}_{mes}.{ext}`. Tests en `PeriodicReportControllerTest`.
- **Frontend**: ✓ — panel "Archivo periódico para socios" en `ReportsView` (`/supervisor/reportes`, mockup 37), ubicado **después** del "Reporte interno de validaciones de acceso". Selector de mes/año/formato (CSV/Excel/PDF), **chips de selección multi-departamento** (envía `departmentNames`) y descarga automática. El botón "Enviar a socio internacional" abre un **modal de vista previa** (de `archivo-periodico/preview`, sin datos personales) con botón **Descargar**; el supervisor descarga el archivo y lo comparte con el socio por el canal externo.
