# HU-08 - REGISTRAR PERSONAL INDIVIDUAL
...
| **Complejidad** | Alta |
| **HU Relacionada** | HU-03, HU-09, HU-28 |
...
El cargo se selecciona del catálogo de cargos (HU-29) mediante una lista desplegable; no se escribe libremente. Del cargo elegido se deriva si el empleado podrá tener usuario del sistema y con qué rol (HU-04).
...
Entonces: el cargo se selecciona del catálogo de cargos mediante una lista desplegable y de él se deriva si el empleado será candidato a usuario del sistema (HU-04)
...
- **Frontend**: ✓ — `RegisterEmployeeView` (`/personal/nuevo`, mockup 42) con validación por campo, notificación con el código generado y carga de fotografía opcional (HU-28). `EmployeeDetailView` (`/personal/:id`, mockup 41) permite gestionar la foto.
