...
- **Backend**: ✓ — `POST /api/permisos` (201 / 400 empleado inactivo / 404 / 409 duplicado empleado+área) y `PATCH /api/permisos/{id}` (edición de fechas y horarios de permisos ACTIVO). `PermissionSchedule` LUN..DOM (HU-26). Tests verdes.
